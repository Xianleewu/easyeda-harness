...
3. For a new project, update `project_spec.json`, pass `node bin/easyeda-gsd.mjs plan` / `gsd_plan_report.json`, pass `spec:schema`, derive `project_contract.json`, define required endpoints in `project_netlist.json`, choose or declare the circuit-pack `pack.mjs` and `cell_manifest.json`, and map deterministic cells/layout policy in `project_assembly.json` before changing cells.
