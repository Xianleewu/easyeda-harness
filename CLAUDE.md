...
3. For a new project, update `project_spec.json`, pass `node bin/easyeda-gsd.mjs plan` / `gsd_plan_report.json`, pass plan-gated `node bin/easyeda-gsd.mjs generate` / `gsd_generate_report.json`, pass `spec:schema`, derive `project_contract.json`, define required endpoints in `project_netlist.json`, choose or declare the circuit-pack `pack.mjs` and `cell_manifest.json`, and map deterministic cells/layout policy in `project_assembly.json` before changing cells.
...
5. Prefer `node bin/easyeda-gsd.mjs accept`; it wraps the safe local gate path. For targeted debugging, run `npm.cmd run spec:schema`, `npm.cmd run spec`, `npm.cmd run contract`, `npm.cmd run contract:netlist`, `npm.cmd run contract:pack`, `npm.cmd run contract:library`, `npm.cmd run contract:cells`, `npm.cmd run contract:assembly`, `npm.cmd run contract:layout`, then `npm.cmd run accept`.
