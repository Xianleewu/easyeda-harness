...
// 断言集成层不变量恒成立:几何(overlaps/wireThruComp/crossings/offgrid)=0、忠实度=0、全模块落地,
...
test('集成 fuzz:60 随机多模块图 — 几何/忠实/落地不变量恒成立(标签仅允许已知 L10 跨模块碰撞)', () => {
	...
	for (let t = 0; t < 60; t++) {
		...
		// 标签:仅允许已知 L10 跨模块同名同 y 碰撞(独立限界);其余 L 规则必须为 0。
		const hard = labelQC(r.model).filter(f => f.severity === 'hard');
		const nonL10 = hard.filter(f => f.rule !== 'L10-dup-named-stub');
		assert.equal(nonL10.length, 0, `${ctx} 非 L10 标签硬伤 ${JSON.stringify(nonL10.slice(0, 2))}`);
		if (hard.length > nonL10.length) l10seen++;
		checked++;
	}
