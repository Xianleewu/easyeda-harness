...
export function runPlexusSynthesize() {
	...
	return {
		...
		geom: { overlaps: g.overlaps.length, wireThruComp: g.wireThruComp.length, wireThruPin: g.wireThruPin.length, offgrid: g.offgrid, offgrid5: g5.offgrid, crossings: g.crossings },
		labelHard,
...
if (import.meta.url === `file://${process.argv[1]}`) {
	...
	// 硬判:重叠/线穿件/线压外部脚/异网交叉/标签硬伤/跨模块连通丢失/导线连通断为 0 才过门(offgrid 暂列软)。
	const hard = out.geom.overlaps + out.geom.wireThruComp + out.geom.wireThruPin + out.geom.crossings + out.labelHard + out.faithHard + out.connHard;
	...
	console.log(`Plexus 合成:placed=${out.placed}/${out.modules} wires=${out.model.wires} flags=${out.model.flags}`
		+ ` | geom overlaps=${out.geom.overlaps} wireThruComp=${out.geom.wireThruComp} wireThruPin=${out.geom.wireThruPin} crossings=${out.geom.crossings} labelHard=${out.labelHard} faithHard=${out.faithHard} connHard=${out.connHard}`
