...
export async function applySource({ robust = false, maxTries = 3 } = {}) {
	...
	const defects = g.overlaps.length + g.wireThruComp.length + g.wireThruPin.length + g.crossings + g.collinear + lh + faith + conn;
	console.log(`投递态质量门:overlaps=${g.overlaps.length} wireThruComp=${g.wireThruComp.length} wireThruPin=${g.wireThruPin.length} crossings=${g.crossings} collinear=${g.collinear} labelHard=${lh} faithHard=${faith} connHard=${conn}（忠实/连通基于投递态线集）`);
	if (dropN) {
