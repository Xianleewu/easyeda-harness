...
//   - 电源/地网:走 power/gnd 符号、EDA 全局同名连通,单符号即可,不在本检查口径。
	...
	for (const net of logical.nets) {
		...
		if (skipped.length) {
			findings.push({
				...
				msg: `跨模块信号网 ${net.name} 触及模块 ${skipped.join(',')} 被跳过 → 连通丢失`,
				where: { net: net.name, skipped },
			});
			continue;
		}
		...
		if (got < mods.size) {
			findings.push({
				...
				msg: `跨模块信号网 ${net.name} 落在 ${mods.size} 模块,输出仅 ${got} 网标(<模块数,某界面缺标签无法逐模块重连)`,
