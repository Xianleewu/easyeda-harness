...
test('expectedLines:记录写入的每条 LINE 段(组+坐标,y 取负),覆盖 cap + 并组', () => {
	const { expectedLines } = buildSource(SRC, r, idByDes);
	// 3 条合成线各 1 段:w1←GND[0,0,10,0]、w2←SIG[40,0,50,0]、溢出 GND 并入 w1[60,0,70,0]
	assert.equal(expectedLines.length, 3);
	const keyset = new Set(expectedLines.map(([g, a, b, c, d]) => `${g}|${a}|${b}|${c}|${d}`));
	assert.ok(keyset.has('w1|0|0|10|0'), 'w1 承载 GND 段');
	assert.ok(keyset.has('w2|40|0|50|0'), 'w2 承载 SIG 段');
	assert.ok(keyset.has('w1|60|0|70|0'), '溢出 GND 段并入 w1');
});

test('expectedLines 与 newSrc 的 LINE 记录精确一致(投递后核对基准真实可核,无虚报)', () => {
	const { newSrc, expectedLines } = buildSource(SRC, r, idByDes);
	const live = new Set(parseSource(newSrc).filter(x => x.head?.type === 'LINE')
		.map(x => `${x.data.lineGroup}|${x.data.startX}|${x.data.startY}|${x.data.endX}|${x.data.endY}`));
	for (const [g, a, b, c, d] of expectedLines)
