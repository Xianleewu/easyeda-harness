...
export function renderSheetOutput(snapshot, outPng = DEFAULT_OUT, opts = {}) {
	...
	/* 真标签通道(src2model 产出的 snapshot.labels,对齐感知):RIGHT_BOTTOM=文字在锚左下[x-w,x]×[y,y+8]。
	 * 无此通道时 wire 上的 NET 标签永远不可见 → 视觉验证失真(2026-09-09 错列误判根因)。 */
	for (const lab of (snapshot.labels || [])) {
		if (!lab || !lab.net) continue;
		const fs2 = 8, w = String(lab.net).length * fs2 * 0.58;
		const al = String(lab.align || '');
		const x1 = al.includes('RIGHT') ? lab.x - w : al.includes('LEFT') ? lab.x : lab.x - w / 2;
		pushText(out, textBoxes, {
			x: x1, y: (lab.y ?? 0) + 8, text: lab.net, fontSize: fs2, anchor: 'start',
			fill: netColor(lab.net), role: 'net-label', owner: lab.net, callout: 'net-label-backplate',
			box: { minX: x1, maxX: x1 + w, minY: lab.y ?? 0, maxY: (lab.y ?? 0) + 8 },
		});
	}
	out.push(`</svg>`);
	const svg = out.join('\n');
