...
| Q7 | **快路径 `wf quick`（最高优先）**：小板专用直达轨 = 引擎构造式布局（`cluster_generate`/`module_repack` 现成件）→ 离线候选 preflight（今日 Q1 已验收的门禁，即快路径的刹车）→ 单次 `wf commit` 写入 → 单次 `wf audit`。无修复循环。**KPI=skill 验收标准：≤25 件板，零细节沟通，≤2 次 live 写入，≤15 分钟，audit 全绿** | engine+live | codex | doing | 已先消除 compile 快路径的跨轮缓存缺陷：从回执绑定的 `workflow-context.json` 原子读取 lint/check 同轮源、报告、网表、几何和引脚归属，红 token 基线可进入修复棘轮，跨文档/源码/规则过期则失败关闭。待完成单命令编排与 KPI 实测。 |
