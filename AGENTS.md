...
Use `docs/agent-runner-guide.md` as the neutral runner contract. Prefer `node bin/easyeda-plexus.mjs ...` for agent workflows; npm scripts remain lower-level debugging and CI building blocks.
...
For a new schematic project, do not draw directly in EasyEDA and then hope the gates catch the result. Start with `node bin/easyeda-plexus.mjs init --pack <pack> --out <project-dir>` to create the editable spec/contract/netlist/assembly scaffold. If `<pack>` is new, this also creates `circuit_packs/<pack>/pack.mjs`, `circuit_packs/<pack>/cell_manifest.json`, and updates the registry. Fill that scaffold until `plexus_plan_report.json` passes. First create or update the project-specific structure that the gates can reason about:
...
Use `node bin/easyeda-plexus.mjs repair` for the read-only grouped repair plan; it is backed by `workflows/repair_loop.mjs`, writes `repair_loop_report.json`, and must not write source files unless a future allowlisted repair mode is explicitly implemented.
...
- `plexus:plan`: `project_spec.json` is realized by the current contract, netlist, assembly, and circuit pack
...
- `plexus:generate`: a passing Plexus plan produced `full_model.json` and `report.json` through deterministic generation
...
```bash
node bin/easyeda-plexus.mjs bridge-check   # verify the bridge is up; prints startup guidance if not
...
- `plexus_generate_report.json` proving deterministic generation was plan-gated
...
2. For a new project directory, run `node bin/easyeda-plexus.mjs init --pack <pack> --out <project-dir>` and use the generated scaffold files plus `circuit_packs/<pack>/` as the editing surface.
3. Derive/update `project_contract.json` from the spec and run `npm.cmd run plexus:plan`, `npm.cmd run design:brief`, `npm.cmd run spec`, and `npm.cmd run contract` until all pass and the brief explains the intended layout before generation.
...
9. Iterate from `next_actions.json`; do not bypass failed findings with manual EasyEDA edits.
   Use `node bin/easyeda-plexus.mjs repair` when you need the grouped repair plan and rerun commands.
10. Write back only through `node bin/easyeda-plexus.mjs apply --gated <project-dir>/project_spec.json`, then validate using live snapshot, real canvas image, DRC, and live shots.
